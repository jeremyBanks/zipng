...
fn anonymous_tuple_to_named() {
    let expected = vec![LongTuple(1, 2, 0), LongTuple(4, 5, 0), LongTuple(7, 8, 0)];
    let src: Vec<(i32, i32)> = expected
        .iter()
        .cloned()
        .map(|LongTuple(x, y, _)| (x, y))
        .collect();
    let dest: Vec<LongTuple> = from_bytes(&to_bytes(&src).unwrap()).unwrap();
    assert_eq!(dest, expected);
}
...
fn tuple_to_struct() {
    ...
    let src: Vec<ShortTuple> = expected
        .iter()
        .cloned()
        .map(|v| ShortTuple(v.x, v.y))
        .collect();
    let dest: Vec<LongStruct> = from_bytes(&to_bytes(&src).unwrap()).unwrap();
    assert_eq!(dest, expected);
}
...
fn tuple_variant_to_struct() {
    ...
    let expected: Vec<Struct> = src
        .iter()
        .map(|&Tuple::Foo(x, y)| Struct::Foo { x, y })
        .collect();
    assert_eq!(dest, expected);
}
...
fn struct_variant_to_newtype_struct() {
    ...
    let expected: Vec<E2> = src
        .iter()
        .map(|&E1::Foo { x, y }| E2::Foo(Foo { x, y }))
        .collect();
    assert_eq!(dest, expected);
}
