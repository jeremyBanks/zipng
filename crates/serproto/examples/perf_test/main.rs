...
fn test_ser_de<T: Serialize + DeserializeOwned>(
    ...
    test_ser_de_detail(
        value,
        &mut checker,
        |buf, val| { buf.write_all(postcard::to_allocvec(val).unwrap().as_ref()).unwrap(); },
        |buf| { postcard::from_bytes(buf).unwrap() },
        "postcard",
    );
    test_ser_de_detail(
        value,
    ...
    test_ser_de_detail(
        value,
        &mut checker,
        |buf, val| serproto::to_writer(buf, val).unwrap(),
        |buf| serproto::from_bytes(buf).unwrap(),
        "serproto",
    );
}
            ...
            list: (0i32..3)
                ...
                .map(|i| benchfb::FooBar {
                    sibling: benchfb::Bar {
                        ...
                        time: 123456 + i as i32,
                        ratio: 3.141519 + i as f32,
                        size: 10000 + i as u16,
                    },
                    name: "Hello, World!".into(),
                    rating: 3.141_543_243_244_554_3 + i as f64,
                    postfix: b'!' + i as u8,
                })
                .collect(),
            initialized: true,
                ...
                .map(|i| protobench::FooBar {
                    sibling: Some(protobench::Bar {
                        parent: Some(protobench::Foo {
                            id: 0xABADCAFEABADCAFE + i as u64,
                            count: 10000 + i as i32,
                            prefix: '@' as i32 + i as i32,
                            length: 1000000 + i as u32,
                        }),
                        time: 123456 + i as i32,
                        ratio: 3.141519 + i as f32,
